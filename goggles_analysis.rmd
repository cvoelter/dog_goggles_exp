...
#more dogs than expected by chance (28/32) approached and looked through the transparent screen first (p = 1.93e-05) 

#if we consider the additional 6 dogs that did not pass the manipulation check, the result holds:
binom.test(28,38) #p-value = 0.005
...
plot_first_look<-ggplot(data=first_looks_data_agg, aes(x=looking_condition, y=mean_fl_con))+
  ...
  ylab("Probability to look first at gaze congruent target")+
...
p_choice<-ggplot(data=first_choice_agg2, aes(x=looking_condition, y=mean))+
  ...
  ylim(0,0.85)+
  xlab("Experimenter gaze cue")+
  ylab("Probability to choose gaze congruent target")+
  theme_classic()
